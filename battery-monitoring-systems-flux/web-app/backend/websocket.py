...
class WebSocketManager:
    ...
    def start(self):
        ...
        try:
            self.logger.info("Starting WebSocket server (sync)")
            # The actual server start is handled by start_websocket_server function
            # This method is just for compatibility with the main system
            self.logger.info("WebSocket server start initiated (sync)")
                    ...
                    if len(df) > 0:
                        ...
                        for _, row in df.iterrows():
                            ...
                            for col, value in row.items():
                                if hasattr(value, 'isoformat'):  # Handle datetime/timestamp objects
        ...
        try:
            while True:
                try:
                    ...
                    if db_stats and db_stats.get('total_records', 0) > 0:
...
# Global WebSocket manager instance for use in other modules
