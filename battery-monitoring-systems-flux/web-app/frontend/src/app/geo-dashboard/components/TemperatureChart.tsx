...
const TemperatureChart = ({ data }: TemperatureChartProps) => {
  ...
  useEffect(() => {
    ...
    const yScale = d3.scaleLinear()
      .domain([d3.min(chartData, d => d.temperature) * 0.95, d3.max(chartData, d => d.temperature) * 1.05])
      .range([height, 0])

    // Create line generator
    const line = d3.line<{ time: number; temperature: number }>()
      .x(d => xScale(d.time))
    ...
    // Add area fill
    const area = d3.area<{ time: number; temperature: number }>()
      .x(d => xScale(d.time))
      .y0(height)
